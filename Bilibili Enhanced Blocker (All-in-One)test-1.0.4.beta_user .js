// ==UserScript==
// @name         Bilibili Enhanced Blocker (All-in-One)test
// @namespace    http://tampermonkey.net/
// @version      1.0.3
// @description  拦截埋点、WebSocket、XHR，修复评论加载异常和Promise错误，并支持日志开关
// @author       fangzny@qwen3
    ...
    (() => {
        ...
        window.fetch = function (input, init) {
            ...
            if (url.includes('/x/v2/reply/wbi/main/fetch')) {
                debugLog('Detected comment fetch to (no longer faking response):', url);
                // Temporarily allow original request to pass through for debugging comment issues
    ...
    (() => {
        ...
        function shouldBlock(url) {
            return (
                ...
                url.includes('hw-v2-web-player-tracker') || // This is for WebSocket, check if it's correctly placed or also an XHR target
                url.includes('ali-web-player-tracker')    // This is for WebSocket, check if it's correctly placed or also an XHR target
                // url.includes('bvc.bilivideo.com') // Removed to prevent video loading issues
            );
        }
